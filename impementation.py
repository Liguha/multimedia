    ...
    def predict(self, X):
        ...
        for x in X:
            ...
            if self.task == "classification":
                ...
                predictions.append(np.bincount(neighbor_labels.astype(int)).argmax())
...
class LogRegression:
    ...
    def fit(self, X, y):
        ...
        self.n_classes = len(np.unique(y)) if self.task == "classification" else 1
        ...
        for i in range(self.n_classes):
            tmp_y = y
            if self.task == "classification":
                tmp_y = np.array([int(abs(yi - i) < 1e-6) for yi in y]).astype(float)
            for _ in range(self.n_iters):
                ...
                dw = (1 / n_samples) * np.dot(X.T, (y_predicted - tmp_y))
                db = (1 / n_samples) * np.sum(y_predicted - tmp_y)
    ...
    def predict(self, X):
        if self.task == "classification":
            ...
            for i in range(self.n_classes):
                linear_model = np.dot(X, self.weights[i]) + self.bias[i]
                y_predicted.append(self._sigmoid(linear_model))
            return np.argmax(np.array(y_predicted), axis=0)
        elif self.task == "regression":
            return np.dot(X, self.weights[0]) + self.bias[0]
...
class GradientBoosting:
    ...
    def predict(self, X):
        ...
        for tree in self.models:
            y_pred += self.learning_rate * tree.predict(X)
        return y_pred if self.task == "regression" else np.round(y_pred)
